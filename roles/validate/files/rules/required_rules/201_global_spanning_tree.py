            ...
            if inventory["vxlan"].get("global", None):
                if inventory["vxlan"].get("global", None).get("spanning_tree", None):
                    ...
                    if root_bridge_protocol == "rpvst+" and not mst_instance_range:
                        results.append(
                            f"vxlan.global.spanning_tree.vlan_range must be set when the spanning tree vxlan.global.spanning_tree.root_bridge_protocol is set to rpvst+."
                        )

                    if root_bridge_protocol == "mst" and not vlan_range:
                        results.append(
                            f"vxlan.global.spanning_tree.mst_instance_range can only be used when the spanning tree vxlan.global.spanning_tree.root_bridge_protocol is set to mst."
                        )
